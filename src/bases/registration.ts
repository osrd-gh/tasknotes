...
export async function registerBasesTaskList(plugin: TaskNotesPlugin): Promise<void> {
	...
	const attemptRegistration = async (): Promise<boolean> => {
		try {
			...
			// Register Agenda view using public API (same factory as Calendar, list-oriented layout)
			const agendaSuccess = registerBasesView(
				plugin,
				"tasknotesAgenda",
				{
			...
			if (!taskListSuccess && !kanbanSuccess && !calendarSuccess && !agendaSuccess && !miniCalendarSuccess) {
...
export function unregisterBasesViews(plugin: TaskNotesPlugin): void {
	...
	try {
		...
		unregisterBasesView(plugin, "tasknotesAgenda", logger);
